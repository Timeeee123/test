...
function ESX.RegisterServerCallback(name, cb)
	...
	-- exports['xcore-callback']:registerCallback(name, cb)
end
...
function Core.SavePlayer(xPlayer, playerId, cb)
	MySQL.prepare('UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `group` = ?, `position` = ?, `inventory` = ?, `loadout` = ?, `health` = ? WHERE `identifier` = ?', {
		...
		'{"health":'..GetEntityHealth(GetPlayerPed(playerId))..',"armour":'..GetPedArmour(GetPlayerPed(playerId))..'}',
		xPlayer.identifier
...
function ESX.GetPlayerFromIdentifier(identifier)
	playerid = Core.playersByIdentifier[identifier]
	return ESX.Players[playerid]
end

function ESX.GetIdentifier(playerId)
	for k,v in ipairs(GetPlayerIdentifiers(playerId)) do
		if string.match(v, 'steam:') then
			local identifier = string.sub(v, 0)
			return identifier
		end
	end
end
...
function ESX.RegisterUsableItem(item, cb)
	...
	ESX.Items[item].usable = true
	Core.UsableItemsCallbacks[item] = cb
	print(('[^2ZC BASE^7][^1REGIS^7] ItemName: [^1%s^0]'):format(item))
end

function ESX.UseItem(source, item, ...)
	if not Core.UsableItemsCallbacks[item] then return end
